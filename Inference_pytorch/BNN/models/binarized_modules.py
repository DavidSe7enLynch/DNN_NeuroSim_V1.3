            ...
            for j in range(self.weight.shape[3]):
                ...
                if numSubArray == 0:
                    ...
                    if self.weight.shape[1] == 3:
                        ...
                        #                                                                    self.vari,
                        #                                                                    device='cuda'))
                        outputPartial = nn.functional.conv2d(input, remainderQ * mask, self.bias, self.stride, self.padding,
                        ...
                        for z in range(self.wl_input):
                            ...
                            outputPartial = nn.functional.conv2d(inputB, remainderQ * mask, self.bias, self.stride,
                                                     self.padding, self.dilation, self.groups)
                            outputDummyPartial = nn.functional.conv2d(inputB, dummyP * mask, self.bias, self.stride,
                                                          self.padding, self.dilation, self.groups)
                            # Add ADC quanization effects here !!!
