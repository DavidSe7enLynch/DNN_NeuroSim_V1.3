...
parser.add_argument('--is_linear', default=1, help='linear or non-linear quantization')
...
# print("args.is_linear = {}".format(args.is_linear))
...
if args.model == 'VGG8':
    ...
    #              '.pth'
    model_path = './log/default/ADCprecision=5/batch_size=64/cellBit=2' \
                 '/dataset=cifar10/decreasing_lr=140,' \
                 '180/detect=0/grad_scale=8/inference=1/is_linear=1/lr=0.01/mode=WAGE/model=VGG8/onoffratio=10/seed' \
                 '=117/subArray=128/t=0/target=0/v=0/vari=0/wl_activate=4/wl_error=8/wl_grad=8/wl_weight=4/best-141' \
                 '.pth'
...
if args.model == 'Llayer':
    ...
    # print("type of correct is " + str(type(correct)) + ", correct " + str(correct))
    # print("len(test_loader.dataset) " + str(len(test_loader.dataset)))
    acc = 100. * correct / len(test_loader.dataset)
    acc1 = 100. * correct / total_pred
    # print("type of acc is " + str(type(acc)) + ", acc " + str(acc))
    # print("type of correct is " + str(type(correct)) + ", correct " + str(correct))
    # print("len(test_loader.dataset) " + str(len(test_loader.dataset)))
    # print("type of acc is " + str(type(acc)) + ", acc " + str(acc))

    accuracy = acc.cpu().data.numpy()
    accuracy1 = acc1.cpu().data.numpy()
    # print("type of accuracy is " + str(type(accuracy)) + ", accuracy " + str(accuracy))
...
print("HRR accuracy = {} / {} = {:.6f}".format(correct, total_pred, accuracy1))
