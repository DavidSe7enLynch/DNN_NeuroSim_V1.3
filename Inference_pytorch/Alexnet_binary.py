...
class AlexNetOWT_BN(nn.Module):
    def __init__(self, args, logger, num_classes=10):
        super(AlexNetOWT_BN, self).__init__()
        self.ratioInfl=3
        self.features = nn.Sequential(
            QConv2d(in_channels = 3, out_channels = int(64*self.ratioInfl), kernel_size = 11,stride=4, padding=2,
                                ...
                                name='QConv2d', model = args.model),
            #BinarizeConv2d(3, int(64*self.ratioInfl), kernel_size=11, stride=4, padding=2),
            nn.MaxPool2d(kernel_size=3, stride=2),
            nn.BatchNorm2d(int(64*self.ratioInfl)),
            nn.Hardtanh(inplace=True),
            QConv2d(in_channels = int(64*self.ratioInfl), out_channels = int(192*self.ratioInfl), kernel_size = 5, padding=2,
                                logger=logger, wl_input = args.wl_activate,wl_activate=args.wl_activate,wl_error=args.wl_error,
                                wl_weight=args.wl_weight,inference=args.inference,onoffratio=args.onoffratio,cellBit=args.cellBit,
                                subArray=args.subArray,ADCprecision=args.ADCprecision,vari=args.vari,t=args.t,v=args.v,detect=args.detect,target=args.target, 
                                name='QConv2d', model = args.model),
            ...
            nn.Hardtanh(inplace=True),
            QConv2d(in_channels = int(192*self.ratioInfl), out_channels = int(384*self.ratioInfl), kernel_size = 3, padding=1,
                                logger=logger, wl_input = args.wl_activate,wl_activate=args.wl_activate,wl_error=args.wl_error,
                                wl_weight=args.wl_weight,inference=args.inference,onoffratio=args.onoffratio,cellBit=args.cellBit,
                                subArray=args.subArray,ADCprecision=args.ADCprecision,vari=args.vari,t=args.t,v=args.v,detect=args.detect,target=args.target, 
                                name='QConv2d', model = args.model),
            ...
            QConv2d(in_channels = int(384*self.ratioInfl), out_channels = int(256*self.ratioInfl), kernel_size = 3, padding=1,
                                logger=logger, wl_input = args.wl_activate,wl_activate=args.wl_activate,wl_error=args.wl_error,
                                wl_weight=args.wl_weight,inference=args.inference,onoffratio=args.onoffratio,cellBit=args.cellBit,
                                subArray=args.subArray,ADCprecision=args.ADCprecision,vari=args.vari,t=args.t,v=args.v,detect=args.detect,target=args.target, 
                                name='QConv2d', model = args.model),
            ...
            QConv2d(in_channels = int(256*self.ratioInfl), out_channels = 256, kernel_size = 3, padding=1,
                                logger=logger, wl_input = args.wl_activate,wl_activate=args.wl_activate,wl_error=args.wl_error,
                                wl_weight=args.wl_weight,inference=args.inference,onoffratio=args.onoffratio,cellBit=args.cellBit,
                                subArray=args.subArray,ADCprecision=args.ADCprecision,vari=args.vari,t=args.t,v=args.v,detect=args.detect,target=args.target, 
                                name='QConv2d', model = args.model),
        ...
        self.classifier = nn.Sequential(
            QLinear(in_features= 256 * 6 * 6, out_features= 4096, 
                                logger=logger, wl_input = args.wl_activate,wl_activate=args.wl_activate,wl_error=args.wl_error,
                                wl_weight=args.wl_weight,inference=args.inference,onoffratio=args.onoffratio,cellBit=args.cellBit,
                                subArray=args.subArray,ADCprecision=args.ADCprecision,vari=args.vari,t=args.t,v=args.v,detect=args.detect,target=args.target, 
                                name='QLinear', model = args.model),
            #BinarizeLinear(256 * 6 * 6, 4096),
            nn.BatchNorm1d(4096),
            nn.Hardtanh(inplace=True),
            #nn.Dropout(0.5),
            QLinear(in_features= 4096, out_features= 4096, 
                                logger=logger, wl_input = args.wl_activate,wl_activate=args.wl_activate,wl_error=args.wl_error,
                                wl_weight=args.wl_weight,inference=args.inference,onoffratio=args.onoffratio,cellBit=args.cellBit,
                                subArray=args.subArray,ADCprecision=args.ADCprecision,vari=args.vari,t=args.t,v=args.v,detect=args.detect,target=args.target, 
                                name='QLinear', model = args.model),
            ...
            nn.Hardtanh(inplace=True),
            #nn.Dropout(0.5),
            QLinear(in_features= 4096, out_features= num_classes, 
                                logger=logger, wl_input = args.wl_activate,wl_activate=args.wl_activate,wl_error=args.wl_error,
                                wl_weight=args.wl_weight,inference=args.inference,onoffratio=args.onoffratio,cellBit=args.cellBit,
                                subArray=args.subArray,ADCprecision=args.ADCprecision,vari=args.vari,t=args.t,v=args.v,detect=args.detect,target=args.target, 
                                name='QLinear', model = args.model),
...
def alexnet_binary(args, logger, pretrained=None, **kwargs):
    num_classes = getattr(kwargs, 'num_classes', 10)
    model = AlexNetOWT_BN(args = args, logger = logger, num_classes = num_classes)
